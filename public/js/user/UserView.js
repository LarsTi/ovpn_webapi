class UserView extends React.Component{
	constructor(props){
		super(props);
		if(!props.apiUrl){
			console.log("Keine API URL gesetzt");
		};
		this.state = {
			refresh: this.refresh
		};
	}
	refresh(){
		fetch(this.props.apiUrl).then( response => {
			if(response.ok){
				response.json().then(this.dataArrived.bind(this));
			}else{
				console.log("Fehler bei fetch!");
			}
		});
	}
	dataArrived(data){
		this.setState({data: data});
		console.log("received user data");
	}
	componentDidMount() {
		this.refresh();
	}
	getHeader(){
		return React.createElement(
			"div", {className: "row"},
			React.createElement(
				"div",{className: "col-2"},
				React.createElement("b", {}, "Vorname")
			),
			React.createElement(
				"div",{className: "col-2"},
				React.createElement("b", {}, "Name")
			),
			React.createElement(
				"div",{className: "col-2"},
				React.createElement("b", {}, "Passwort")
			),
			React.createElement(
				"div",{className: "col-1"},
				React.createElement("b", {}, "Org")
			),
			React.createElement(
				"div",{className: "col-3"},
				React.createElement("b", {}, "Mail")
			),
			React.createElement(
				"div",{className: "col-2"}
			)

		);
	}
	render(){
		return React.createElement(
			"div", {className: "container"},
			this.props.app.getTitle("Benutzer pflegen"),
			this.props.app.getSeperator(),
			this.getHeader(),
			this.props.app.getSeperator(),
			this.props.app.getSeperator(),
			React.createElement(UserNew, {
				apiUrl: this.props.apiUrl, 
				app: this.props.app,
				refresh: this.refresh.bind(this)
			}),
			this.props.app.getSeperator(),
			this.state && this.state.data && this.state.data.map( function(user){
				user.refresh = this.refresh.bind(this);
				user.key = "user-" + user.ID;
				user.apiUrl = this.props.apiUrl;
				user.app = this.props.app;
				return React.createElement(
					User,
					user
				);
			}.bind(this))
		);
	}
}
